...
# Following function created by mullerb@musc.edu
AC_DEFUN([AX_PYTHON_PACKAGES_DIR],
[
 AC_MSG_CHECKING(for Python packages directory)
 if test -z "$PYTHON"
 then
...
print sysconfig.get_python_lib()"` || py_error="yes"
 if test "$py_error" = "yes"
 then
   AC_MSG_RESULT(no - an error occurred)
 else
   AC_MSG_RESULT($pyval)
   PYTHON_PACKAGES_DIR="$pyval"
   AC_SUBST(PYTHON_PACKAGES_DIR)
 fi
])
